    ...
    <main className="flex justify-center min-h-screen text-base px-8 sm:px-4 py-12 sm:py-28 2xl:pt-0 2xl:items-center">
      <div className="flex flex-col space-y-6 max-w-3xl w-full">
        <h3 className="font-serif font-semibold text-2xl sm:text-3xl">
          Session-Based Authentication with Tokenless CSRF Defense: A Modern Approach to Secure Web
          Authentication
        </h3>
        <div className="space-y-4">
          <p className="leading-6 text-justify">
            Session-based authentication stores a user&apos;s identity on the server and relies on
            cookies the browser automatically includes with every matching request — which
            introduces the risk of Cross-Site Request Forgery (CSRF). The tokenless defense pattern
            mitigates this not with a shared secret, but by enforcing browser-level constraints that
            attackers cannot satisfy: requiring non-simple Content-Types to force CORS preflight,
            validating Origin and Referer headers, and leveraging Fetch Metadata headers that
            browsers set and attackers cannot forge.
            {!showMore && "... "}{" "}
            <span>
              <Button
                variant="link"
          ...
          <div className={cn("space-y-4", showMore ? "" : "hidden")}>
            <p>
              ...
              site — ports are excluded from the same-site determination. This means Sec-Fetch-Site
              resolves to same-site for all requests between them, the session cookie flows freely,
              and every security middleware behaves exactly as it will in production. The only
              intentional relaxations are that Secure is not set on the cookie since there is no
              HTTPS, and the Domain attribute is omitted since localhost does not require it.
            </p>
            <p>
              ...
              domain — a phishing link, a cross-site form, or a scripted fetch — is rejected at the
              Fetch Metadata layer before the handler is ever reached.
            </p>
            <p>
              If the frontend and API were on entirely separate domains — say example.com and
              ...
              as a compensating control — precisely the complexity this same-site setup is designed
              to avoid.
            </p>
          </div>
        </div>
